...
EGVAR(end_conditions,notificationTime) = 30 MINUTES;  // Send a notification to each player X seconds before the time limit (-1: disabled)
...
GVARMAIN(taskThreshold) = 60;  // Percentage of the tasks that have to be completed
...
EGVAR(respawn,timer) = 240;  // Respawn timer in seconds
...
EGVAR(intro_text,title) = "Road To Electro";  // Title
EGVAR(intro_text,date) = "2024-11-30 13:00";  // Date
EGVAR(intro_text,location) = "Komarovo docks, 3km west of Electro";  // Location
