...
EGVAR(end_conditions,extTask) = "tsk_ext";  // Extraction task that will be automatically completed (empty string disables this functionality)
...
EGVAR(respawn,availableWaves) = 100;  // Number of available respawn waves (MANUAL respawn waves only)
...
GVARMAIN(serverViewDistance) = 450;  // Server view distance (= AI view distance)
...
EGVAR(ai_skills,skillSet) = [
    0.6,  // General (Higher = Better)
    0.7,  // Commanding (Higher = Better)
...
EGVAR(ao_limit,timerLand) = 50;  // Timer for any land based vehicle/unit (-1: disabled)
...
EGVAR(intro_text,title) = "Operation Borros";  // Title
EGVAR(intro_text,date) = "10 SEP 2000";  // Date
EGVAR(intro_text,location) = "Laghisola";  // Location
...
EGVAR(reinsertion,TPPoles) = ["MF_tpPole_1", "MF_tpPole_2","sniper_ow","Z_1"];  // Teleport pole object(s)
