...
EGVAR(ai_skills,skillSet) = [
    0.8,  // General (Higher = Better)
    0.8,  // Commanding (Higher = Better)
    0.8,  // Courage (Higher = Better)
    0.4,  // Aiming Accuracy (Higher = Better)
    0.4,  // Aiming Shake (Higher = Less)
    0.4,  // Aiming Speed (Higher = Faster)
    0.8,  // Reload Speed (Higher = Faster)
    0.8,  // Spotting Distance (Higher = Further)
    0.8   // Spotting Time (Higher = Faster)
];
...
EGVAR(intro_text,title) = "Paint The Town Blue";  // Title
EGVAR(intro_text,date) = "1995";  // Date
...
EGVAR(supply_drop,supplyDropPlane) = "LOP_UN_Mi8MT_Cargo";  // Plane or helicopter used for the supply drop
