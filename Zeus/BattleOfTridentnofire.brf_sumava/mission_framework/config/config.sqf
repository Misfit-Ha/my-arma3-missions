...
// View distances
GVARMAIN(playerViewDistance) = 3000;  // Player view distance
...
EGVAR(ai_skills,skillSet) = [
    0.9,  // General (Higher = Better)
    0.9,  // Commanding (Higher = Better)
    0.9,  // Courage (Higher = Better)
    0.9,  // Aiming Accuracy (Higher = Better)
    0.9,  // Aiming Shake (Higher = Less)
    0.9,  // Aiming Speed (Higher = Faster)
    0.9,  // Reload Speed (Higher = Faster)
    0.9,  // Spotting Distance (Higher = Further)
    0.9   // Spotting Time (Higher = Faster)
];
...
EGVAR(intro_text,title) = "Battle Of Trident";  // Title
EGVAR(intro_text,date) = "Early 1970s";  // Date
EGVAR(intro_text,location) = "Eastern Europe";  // Location
EGVAR(intro_text,delay) = 15;  // Delay after loading in


// JIP
GVARMAIN(moduleJIP) = true;  // Coop & TvT
EGVAR(jip,jipTimer) = 6 MINUTES;  // For how long the TP is available after joining the mission
...
// Mark custom objects
GVARMAIN(moduleMarkObjects) = true;  // Coop & TvT
