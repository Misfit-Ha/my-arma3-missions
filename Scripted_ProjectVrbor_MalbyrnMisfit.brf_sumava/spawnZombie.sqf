...
selectRandom allPlayers spawn {
    ...
    while {_zombieSpawnGroupCount > 0} do {
        ...
        _startingPosFromObject = [[[getPos _this, 100]], ["water",[getPos _this, 50]]] call BIS_fnc_randomPos;
        _grpup = [_startingPosFromObject, INDEPENDENT, ["I_Survivor_F", "I_Survivor_F", "I_Survivor_F", "I_Survivor_F"]] call BIS_fnc_spawnGroup;
        _un1 = units _grpup select 0;
        _un2 = units _grpup select 1;
        _un3 = units _grpup select 2;
        _un4 = units _grpup select 3;
        ...
        waitUntil {sleep 0.5; (!(alive _un1) and !(alive _un2) and !(alive _un3) and !(alive _un4))};
