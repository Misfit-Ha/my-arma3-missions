...
briefingName = "[TFI] FUBAR";  // Mission name on the mission selection screen (use [CO-XX] or [TVT-XX] where XX is the max. player number)
overviewText = "German forces trying to stop the ChDKZ attack on Chernarus. // Assets: 1x Puma ifv (callsign: HAMMER) // Respawns: unlimited, Wave // Arsenal mode: Alt Loadouts // Time limit: disabled // Staging: disabled // JIP: enabled"; // Mission info on the mission selection screen (e.g. mission assets, factions, time limit etc.)

// Loading screen settings
onLoadName = "FUBAR";  // Name of the operation on the loading screen (you can drop the [CO-XX] part here)
