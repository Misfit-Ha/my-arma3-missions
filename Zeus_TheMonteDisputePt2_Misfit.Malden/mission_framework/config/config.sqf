...
EGVAR(respawn,timer) = 1;  // Respawn timer in seconds
...
EGVAR(respawn,availableWaves) = 5;  // Number of available respawn waves (MANUAL respawn waves only)
...
EGVAR(ai_skills,skillSet) = [
    0.7,  // General (Higher = Better)
    0.7,  // Commanding (Higher = Better)
    0.7,  // Courage (Higher = Better)
    0.7,  // Aiming Accuracy (Higher = Better)
    0.7,  // Aiming Shake (Higher = Less)
    0.7,  // Aiming Speed (Higher = Faster)
    0.7,  // Reload Speed (Higher = Faster)
    0.7,  // Spotting Distance (Higher = Further)
    0.7   // Spotting Time (Higher = Faster)
];
...
EGVAR(intro_text,title) = "THE MONTE DISPUTE PT2";  // Title
EGVAR(intro_text,date) = "EARLY 2030s";  // Date
EGVAR(intro_text,location) = "MALDEN";  // Location
